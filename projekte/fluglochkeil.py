...
keil = Box(
    Vector(-außen_breite / 2, -außen_tiefe / 2, 0) + offset,
    Vector(außen_breite / 2, außen_tiefe / 2, außen_höhe) + offset,
)

aussparung1 = Box(
    Vector(keil.e - aussparung_höhe, keil.n - 30 - aussparung_tiefe, 0),
    Vector(keil.e + aussparung_höhe, keil.n - 30, außen_höhe),
    [Vector(6, 6, 0)],
)
keil.solid = keil.solid.cut(aussparung1.solid)
aussparung2 = Box(
    Vector(keil.c, keil.s + außen_höhe, 0), Vector(keil.e, keil.s, außen_höhe)
)
...
aussparung1 = Box(
    Vector(keil.e - aussparung_höhe, keil.s + 30, 0),
    Vector(keil.e + aussparung_höhe, keil.s + 30 + aussparung_tiefe, außen_höhe),
    [Vector(6, 6, 0)],
)
keil.solid = keil.solid.cut(aussparung1.solid)
aussparung2 = Box(
    Vector(keil.w, keil.n - außen_höhe, keil.u), Vector(keil.e, keil.n, keil.g)
)
keil.solid = keil.solid.cut(aussparung2.solid)
...
loch = Polyhedron(
    Polygon(
        Vector(aussparung1.c, aussparung1.m, aussparung1.g),
        loch_durchmesser / 2,
        Vector(1, 0, 0),
    ),
    Polygon(
        Vector(keil.w, aussparung1.m, aussparung1.g),
        loch_durchmesser / 2,
        Vector(1, 0, 0),
    ),
)
