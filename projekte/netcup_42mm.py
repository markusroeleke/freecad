...
flange_bottom_inner_dia = bottom_outer_dia - 2 * inner_wall_thickness  # 40mm - hier sitzt Flansch innen auf
...
flange_outer_cyl = Part.makeCylinder(
    flange_outer_radius, flange_height,
    Vector(0, 0, -flange_height), Vector(0, 0, 1)
)

# Innenloch des Flansches
flange_hole_cyl = Part.makeCylinder(
    flange_inner_hole_radius, flange_height + 2,
    Vector(0, 0, -flange_height - 1), Vector(0, 0, 1)
)
...
outer_cone = Part.makeCone(
    bottom_outer_radius, top_outer_radius,
    height,
...
inner_cone = Part.makeCone(
    bottom_inner_radius, top_inner_radius,
...
bottom_plate_outer = Part.makeCylinder(
    bottom_outer_radius, inner_wall_thickness,
    Vector(0, 0, 0), Vector(0, 0, 1)
)

bottom_plate_inner = Part.makeCylinder(
    bottom_inner_radius, inner_wall_thickness,
    Vector(0, 0, 0), Vector(0, 0, 1)
)
...
upper_plate_inner = Part.makeCylinder(
    top_inner_radius_hole, inner_wall_thickness,
    Vector(0, 0, height-inner_wall_thickness), Vector(0, 0, 1)
)
...
print(f"  Becher: {bottom_outer_dia}mm ⌀ unten, {top_outer_dia}mm ⌀ oben, {height}mm hoch")
...
Mesh.export([FreeCAD.getDocument(doc_name).getObject('NetCup44')], f"/Users/Markus/Documents/Projekte/FreeCAD/stl/NetCup44.stl")
