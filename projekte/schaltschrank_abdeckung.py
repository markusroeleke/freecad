...
höhe = 10
Abdeckung = Box(Vector(-länge / 2, -breite / 2, 0), Vector(länge / 2, breite / 2, höhe))

Falz = Box(
    Vector(-länge / 2 + rand, -breite / 2 + rand, höhe),
    Vector(länge / 2 - rand, breite / 2 - rand, höhe + wandung),
)
Aussparung = Box(
    Vector(-länge / 2 + rand * 2, -breite / 2 + rand * 2, wandung),
...
for x_offset, y_offset in [
    (
        Abdeckung.w + rand + loch_durchmesser / 2,
        Abdeckung.n - rand - loch_durchmesser / 2,
    ),
    (Abdeckung.c, Abdeckung.n - rand - loch_durchmesser / 2),
    (
        Abdeckung.e - rand - loch_durchmesser / 2,
        Abdeckung.n - rand - loch_durchmesser / 2,
    ),
    (
        Abdeckung.w + rand + loch_durchmesser / 2,
        Abdeckung.s + rand + loch_durchmesser / 2,
    ),
    ...
    loch = Cylinder(
        "loch", loch_durchmesser, -höhe, Vector(x_offset, y_offset, höhe + wandung)
    )
