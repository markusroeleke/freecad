...
ts35_body = Box(
    ...
    Vector(
        TS35_WIDTH_MIN / 2, y_offset + TS35_HEIGHT, HOLDER_LENGTH
    ),  # East, North, Up
)
...
ts35_groove = Box(
    ...
    Vector(
        TS35_WIDTH_MAX / 2, y_offset + TS35_THICKNESS, HOLDER_LENGTH
    ),  # East, North, Up
)
...
ts35_groove_swing = Box(
    ...
    Vector(
        x_offset + TS35_THICKNESS, y_offset + TS35_GROOVE_HIGHT, HOLDER_LENGTH
    ),  # East, North, Up
)
...
screw_driver = Box(
    ...
    Vector(
        main_body.w, main_body.s + SCREW_DRIVER_HEIGHT, HOLDER_LENGTH
    ),  # East, North, Up
)

screw_driver_hole = Box(
    Vector(
        main_body.w - SCREW_DRIVER_HOLE_THINKNESS,
        screw_driver.s + SCREW_DRIVER_HOLE_THINKNESS,
        SCREW_DRIVER_HOLE_THINKNESS,
    ),  # West, South, Down
...
# ecken abrunden
main_body.solid = main_body.solid.makeFillet(
    1.5, [main_body.solid.Edge9, main_body.solid.Edge59]
)
...
rc_box_holder_body = Box(
    ...
    Vector(
        x_offset + RC_BOX_HOLDER_HIGHT, main_body.n + RC_BOX_HOLDER_WIDTH, HOLDER_LENGTH
    ),  # East, North, Up
)
...
rc_box_body = Box(
    ...
    Vector(
        x_offset + RC_BOX_HIGHT,
        y_offset + RC_BOX_WIDTH + RC_BOX_HOLDER_THICKNESS,
        z_offset + RC_BOX_LENGTH,
    ),  # East, North, Up
    # [Vector(1,1,1)]
)
...
rc_box_slip_body = Box(
    ...
    Vector(
        x_offset + RC_BOX_SLIP_HIGHT / 2, y_offset + RC_BOX_SLIP_WIDTH, rc_box_body.d
    ),  # East, North, Up
    [Vector(2, 2, 2)],
)
rc_box_back_body = Box(
    ...
    Vector(
        x_offset + RC_BOX_HOLDER_HIGHT / 2,
        y_offset + RC_BOX_BACK_WIDTH,
        rc_box_body.d + RC_BOX_HOLDER_THICKNESS * 2,
    ),  # East, North, Up
    [Vector(2, 2, 2)],
)
...
rc_box_back_notch = Box(
    Vector(
        x_offset - RC_BOX_NOTCH_HIGHT / 2, y_offset, rc_box_body.d
    ),  # West, South, Down
    Vector(
        x_offset + RC_BOX_NOTCH_HIGHT / 2,
        y_offset + RC_BOX_BACK_WIDTH,
        rc_box_body.d + RC_BOX_HOLDER_THICKNESS * 2,
    ),  # East, North, Up
)
...
notch = Polyhedron(
    Polygon(
        Vector(
            rc_box_slip_body.c,
            rc_box_slip_body.s + RC_BOX_SLIP_HOLE,
            rc_box_slip_body.u,
        ),
        notch_diameter / 2,
        Vector(0, 0, 1),
...
# ecken abrunden
rc_box_holder_body.solid = rc_box_holder_body.solid.makeFillet(
    1.5,
    [
